...
public class TextAttachmentUploadDouble {
	int editorId;
	...
	TextAttachmentUploadDouble(MainActivity context, String textMarkId, int editorId,
			ArrayList<Object> array) {
	...
	public void start() {
			...
			if (isCont && isContNull) {
				...
				if (isTextCont && isTextContNull) {
					...
					if (location + 1 < array.size()) {
						location++;
						TextAttachmentUploadDouble.this.start();
					} else {
						context.textFinish();
					}
					return;
		...
		Log.i("test_tag", "Text  markEditId = " + editorId + " || id" + "= "
				+ textMarkId);
		...
		http.send(HttpRequest.HttpMethod.POST, url, paramsUtil,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						Log.i("test_tag", "textAttach responseInfo.result="
								+ responseInfo.result);
