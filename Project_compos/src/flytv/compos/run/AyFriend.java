...
public class AyFriend extends BaseActivity implements IXListViewListener,
	...
	protected void findViewById() {
		...
		baseAdapter.setUser(false);
		baseAdapter.setItemCommentClickLister(this);
		xListView.setAdapter(baseAdapter);
		itemListStu = loginInfo.subjectList;
		ivTitleName.setText(itemListStu.get(seleIndex).getName());
		
		if(loginInfo.getUserType().equals("2")){
			ivTitleName.setEnabled(true);
			button_grade.setVisibility(View.VISIBLE);
		}else{
			ivTitleName.setEnabled(false);
			button_grade.setVisibility(View.GONE);
		}
	}
	...
	void ref(final int typeId) {
		httpUrl = R.string.compos_circle_all_list;
		if (typeId == 0) {
			page = 1;
			index = 0;
		} else if (typeId == 1) {
	...
	public void onBottomDeleLog(final int position) {
		final Circle circle = items.get(position);
		AlertHelp.showDelete(AyFriend.this, position,circle.getTitle(), this);
	...
	public void onBottomEditLog(int position) {
		// TODO Auto-generated method stub
		Intent intent  = new Intent(AyFriend.this,AyFriendUser.class);
