...
public class XListView extends ListView implements OnScrollListener {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (getFirstVisiblePosition() == 0
					&& (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) { // the
				// first item is showing, header has shown or pull down.
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		int itemsLastIndex = 0; // 数据集最后一项的索引
		itemsLastIndex = this.getAdapter().getCount() - 3;
		lastIndex = itemsLastIndex + 1; // 加上底部的loadMoreView项
