...
import flytv.ext.view.CircularProgressBar.OnProgressChangeListener;

/**
 * 中间带有进度百分比文字显示的环形进度条
 * 
 * <p>
 * 该类是对 CircularProgressBar（环形进度条）类的一个包装，添加了 文字显示部分
 * </p>
 * <p>
 * 实现了 进度条进度改变的监听 {@link OnProgressChangeListener}，从而得到进度的百分比，然后显示到 TextView 上
 * </p>
 * 
...
public class RateTextCircularProgressBar extends FrameLayout implements
	...
	private void init() {
		...
		mCircularProgressBar.setPrimaryColor(getContext().getResources()
				.getColor(R.color.text_loading_color));
		mCircularProgressBar.setBackgroundColor(getContext().getResources()
				.getColor(R.color.text_bg_color));
		...
		mRateText.setTextColor(this.getContext().getResources()
				.getColor(android.R.color.white));
		mRateText.setTextSize(40);
		TextPaint textPaint = mRateText.getPaint();
